# @gaozh1024/rn-observatory 0.3.0 Release Notes

发布日期：2026-05-20

`0.3.0` expands `rn-observatory` from diagnostics-only capture into lightweight, consent-aware product analytics while keeping the existing error capture APIs compatible.
...
- Existing crash capture payloads remain compatible with the current app-observatory ingest endpoint: `POST /api/app-observatory/events`.
...
- `npm pack --dry-run --workspace @gaozh1024/rn-observatory`
