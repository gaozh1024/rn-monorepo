# @gaozh1024/rn-health 0.1.0 Release Notes

发布日期：2026-05-17

## 新增

- 新增 `@gaozh1024/rn-health`，提供 Expo / React Native App 健康监控 MVP。
- 支持 `AppHealthProvider`、`useAppHealth`、`createAppHealthClient`。
...
- `rn-kit` 本身不依赖 `rn-health`，未接入时行为不变

## 注意事项

- 默认 `MemoryHealthStorage` 不适合生产持久化；生产项目应注入 AsyncStorage / MMKV 等持久化 storage adapter。
