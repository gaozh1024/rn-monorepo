...
export interface AxiosHealthInterceptorOptions {
  capture4xx?: boolean;
...
export function installAxiosHealthInterceptor(
  axiosInstance: AxiosLike,
  reporter: AppHealthReporter,
  options: AxiosHealthInterceptorOptions = {}
) {
...
function shouldCaptureAxiosError(error: unknown, options: AxiosHealthInterceptorOptions) {
