...
import { installAxiosObservatoryInterceptor } from '..';
import type { AppObservatoryReporter, AxiosLike } from '..';
...
describe('installAxiosObservatoryInterceptor', () => {
  it('captures 5xx axios response errors', async () => {
    const health = reporter();
    const { instance, reject } = axiosLike();
    installAxiosObservatoryInterceptor(instance, health, { tags: { api: 'orders' } });
    const error = {
      message: 'server down',
  ...
  it('skips 4xx axios errors by default', async () => {
    const health = reporter();
    const { instance, reject } = axiosLike();
    installAxiosObservatoryInterceptor(instance, health);
  ...
  it('captures network errors and ejects interceptor on dispose', async () => {
    const health = reporter();
    const { instance, reject } = axiosLike();
    const dispose = installAxiosObservatoryInterceptor(instance, health);
